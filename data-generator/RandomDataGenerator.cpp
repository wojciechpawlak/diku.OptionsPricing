...
struct RandValuation
{
    ...
    RandValuation(const int maturity, const int termStepCount, const real meanReversionRate, const bool skewed, const int yieldCurveCount)
    {
        TermUnit = 360;
...
void writeYieldCurvesToFile(vector<RandValuation> &randValuations,
...
void distributeYieldCurves(vector<RandYieldCurve> &yieldCurves, const long numYieldCurves)
{
    long currentNumYieldCurves = 0;
    while (currentNumYieldCurves < numYieldCurves)
    {
        int maturity = randIntInRange(1, 30);
        real reversionRate = getRRByWidthRange(7, 107);
        RandYieldCurve yc{};
        yieldCurves.push_back(yc);
        currentNumYieldCurves++;
    }
}

void distribute_0(vector<RandValuation> &valuations, const long numOptions, vector<RandYieldCurve> &yieldCurves, string path)
{
    long currentNumOptions = 0;

    while (currentNumOptions < numOptions)
    {
        RandValuation o(9, 12, 0.1, false, yieldCurves.size());
        addOption(valuations, o, currentNumOptions);
    }

    string filename = "0_UNIFORM_";
    filename.append(to_string(numOptions));
    string dataFile = path + filename + ".in";
    writeYieldCurvesToFile(valuations, yieldCurves, dataFile, 0, numOptions, 0, currentNumOptions, 0);
    cout << "finished writing to " << dataFile << endl;
}

void distribute_1(vector<RandValuation> &valuations, const long numOptions, vector<RandYieldCurve> &yieldCurves, string path)
{
    long currentNumOptions = 0;
    ...
    while (currentNumOptions < numOptions)
    {
        ...
        RandValuation o(maturity, 12, reversionRate, false, yieldCurves.size());
        addOption(valuations, o, currentNumOptions);
    }
    ...
    filename.append(to_string(numOptions));
    string dataFile = path + filename + ".in";
    writeYieldCurvesToFile(valuations, yieldCurves, dataFile, 1, numOptions, 0, currentNumOptions, 0);
    cout << "finished writing to " << dataFile << endl;
}

void distribute_2(vector<RandValuation> &valuations, const long numOptions, vector<RandYieldCurve> &yieldCurves, string path)
{
    long currentNumOptions = 0;
    ...
    {
        real reversionRate = getRRByWidthRange(7, 511);
        RandValuation o(9, 12, reversionRate, false, yieldCurves.size());
        addOption(valuations, o, currentNumOptions);
    }

    string filename = "2_RANDCONSTHEIGHT_";
    filename.append(to_string(numOptions));
    string dataFile = path + filename + ".in";
    writeYieldCurvesToFile(valuations, yieldCurves, dataFile, 2, numOptions, 0, currentNumOptions, 0);
    cout << "finished writing to " << dataFile << endl;
}

void distribute_3(vector<RandValuation> &valuations, const long numOptions, vector<RandYieldCurve> &yieldCurves, string path)
{
    long currentNumOptions = 0;
    ...
    {
        int maturity = randIntInRange(1, 100);
        RandValuation o(maturity, 12, 0.1, false, yieldCurves.size());
        addOption(valuations, o, currentNumOptions);
    }

    string filename = "3_RANDCONSTWIDTH_";
    filename.append(to_string(numOptions));
    string dataFile = path + filename + ".in";
    writeYieldCurvesToFile(valuations, yieldCurves, dataFile, 3, numOptions, 0, currentNumOptions, 0);
    cout << "finished writing to " << dataFile << endl;
}

void distribute_4(vector<RandValuation> &valuations, const long numOptions, const int skewPerc, vector<RandYieldCurve> &yieldCurves, string path)
{
    long currentNumOptions = 0;
    ...
    while (currentNumOptions < (skewPerc / (real)100) * numOptions)
    {
        ...
        RandValuation o(maturity, 12, reversionRate, true, yieldCurves.size());
        addOption(valuations, o, currentNumOptions);
    }
    ...
    filename.append(to_string(numOptions));
    string dataFile = path + filename + ".in";
    writeYieldCurvesToFile(valuations, yieldCurves, dataFile, 4, numOptions, skewPerc, currentNumOptions, currentNumSkewOptions);
    cout << "finished writing to " << dataFile << endl;
}

void distribute_5(vector<RandValuation> &valuations, const long numOptions, const int skewPerc, vector<RandYieldCurve> &yieldCurves, string path)
{
    long currentNumOptions = 0;
    ...
    while (currentNumOptions < (skewPerc / (real)100) * numOptions)
    {
        ...
        RandValuation o(100, 12, reversionRate_skew, true, yieldCurves.size());
        addOption(valuations, o, currentNumOptions);
    }
    ...
    string filename = "5_SKEWEDCONSTHEIGHT_";
    filename.append(to_string(numOptions));
    string dataFile = path + filename + ".in";
    writeYieldCurvesToFile(valuations, yieldCurves, dataFile, 5, numOptions, skewPerc, currentNumOptions, currentNumSkewOptions);
    cout << "finished writing to " << dataFile << endl;
}

void distribute_6(vector<RandValuation> &valuations, const long numOptions, const int skewPerc, vector<RandYieldCurve> &yieldCurves, string path)
{
    long currentNumOptions = 0;
    ...
    while (currentNumOptions < (skewPerc / (real)100) * numOptions)
    {
        ...
        RandValuation o(maturity, 12, reversionRate_skew, true, yieldCurves.size());
        addOption(valuations, o, currentNumOptions);
    }
    ...
    string filename = "6_SKEWEDCONSTWIDTH_";
    filename.append(to_string(numOptions));
    string dataFile = path + filename + ".in";
    writeYieldCurvesToFile(valuations, yieldCurves, dataFile, 6, numOptions, skewPerc, currentNumOptions, currentNumSkewOptions);
    cout << "finished writing to " << dataFile << endl;
}
...
int main(int argc, char *argv[])
{
    ...
    options
        .add_options()
        ("h,help", "Print help")
        ("t,type", "Dataset type, options: 0: uniform, 1: random, 2: random fixed height, 3: random fixed width, 4: skewed, 5: skewed fixed height, 6: skewed fixed width", cxxopts::value<int>(dataType)->default_value("-1"))
        ("n,totalOptions", "Number of valuation", cxxopts::value<long>(totalValuations)->default_value("1"))
        ("y,totalYieldCurves", "Number of yield curves", cxxopts::value<long>(totalYieldCurves)->default_value("1"))
        ("s,skewPerc", "Percentage of skewed valuations", cxxopts::value<int>(skewPercent)->default_value("1"))
    ...
    vector<RandYieldCurve> yieldCurves;
    distributeYieldCurves(yieldCurves, totalYieldCurves);
    ...
    {
    case 0:
        distribute_0(randValuations, totalValuations, yieldCurves, outputPath);
        break;
    case 1:
        distribute_1(randValuations, totalValuations, yieldCurves, outputPath);
        break;
    case 2:
        distribute_2(randValuations, totalValuations, yieldCurves, outputPath);
        break;
    case 3:
        distribute_3(randValuations, totalValuations, yieldCurves, outputPath);
        break;
    case 4:
        distribute_4(randValuations, totalValuations, skewPercent, yieldCurves, outputPath);
        break;
    case 5:
        distribute_5(randValuations, totalValuations, skewPercent, yieldCurves, outputPath);
        break;
    case 6:
        distribute_6(randValuations, totalValuations, skewPercent, yieldCurves, outputPath);
        break;
    default:
        ...
        distribute_0(rand0, totalValuations, yieldCurves, outputPath);
        distribute_1(rand1, totalValuations, yieldCurves, outputPath);
        distribute_2(rand2, totalValuations, yieldCurves, outputPath);
        distribute_3(rand3, totalValuations, yieldCurves, outputPath);
        distribute_4(rand4, totalValuations, skewPercent, yieldCurves, outputPath);
        distribute_5(rand5, totalValuations, skewPercent, yieldCurves, outputPath);
        distribute_6(rand6, totalValuations, skewPercent, yieldCurves, outputPath);
    }
