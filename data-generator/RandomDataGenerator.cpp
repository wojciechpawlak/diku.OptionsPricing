...
struct RandValuation
{
    ...
    RandValuation(const real maturity, const int termStepCount, const real meanReversionRate, const int yieldCurveCount, const bool skewed, const bool noCashflows, const bool europeanOnly)
        : Maturity(maturity), TermStep(termStepCount), MeanReversionRate(meanReversionRate), Skewed(skewed), NoCashflows(noCashflows), EuropeanOnly(europeanOnly)
    {
        TermUnit = 360;
        ...
        computeCashflows();
        Spread = NoCashflows ? zero : (real)randIntInRange(1, 1000) / 100;
    ...
    /**
     * For zero-vopon bond: 1 cashflow: Repayment at Height step of 100.0 and coupon 0.0
     */
    void computeCashflows()
    {
        if (NoCashflows)
        {
            Coupons.push_back(zero);
    ...
    /**
     * For European: only 1 exercise data during the maturityeriod of the bond
     */
    void computeOptionality()
    {
        if (EuropeanOnly)
        {
            ExerciseStepFrequency = 1;
            int exerciseStep = randIntInRange(1, Height - 1);
            FirstExerciseStep = exerciseStep;
            LastExerciseStep = exerciseStep;
        }
        else
        {
 ...
 */
class StdNormDistGenerator {
    default_random_engine generator;
    normal_distribution<real> distribution;
    int min;
    int max;
public:
    StdNormDistGenerator(real mean, real stddev, int min, int max, default_random_engine generator) :
        distribution(mean, stddev), generator(generator), min(min), max(max)
    {}

    StdNormDistGenerator(int min, int max, default_random_engine generator) :
        distribution((real)(min + max) / 2, (real)(max - min) / 6), generator(generator), min(min), max(max)
    {}

    int operator ()() {
        while (true) {
            int number = (int)round(this->distribution(generator));
...
public:
    RandomDataGenerator(unsigned int randomSeed, long totalValuations, long totalYieldCurves, string outputPath, int timeStep, int termUnit, int skewPercent, bool noCashflows, bool onlyEuropean)
        : totalValuations(totalValuations), totalYieldCurves(totalYieldCurves), outputPath(outputPath), timeStep(timeStep), skewPercent(skewPercent), noCashflows(noCashflows), onlyEuropean(onlyEuropean)
    {
        termUnitsInYearCount = (int)ceil((real)year / termUnit);
    ...
    void distribute_0_UNIFORM(const int constHeight, const int constWidth)
    {
        ...
        {
            RandValuation val(constMaturity, timeStep, constMeanReversionRate, randYieldCurves.size(), false, noCashflows, onlyEuropean);
    ...
    void distribute_1_RAND(const int minHeight, const int maxHeight, const bool isHeightNormalDist, const int minWidth, const int maxWidth, const bool isWidthNormalDist)
    {
        long currentNumOptions = 0;

        vector<int> heights(totalValuations);
        vector<int> widths(totalValuations);

        // Get all heights
        if (isHeightNormalDist)
        {
            StdNormDistGenerator heightNormDist(minHeight, maxHeight, engine);
            generate(heights.begin(), heights.end(), [&heightNormDist]() { return heightNormDist(); });
        }
        else
        {
            uniform_int_distribution<int> heightUniformDist(minHeight, maxHeight);
            generate(heights.begin(), heights.end(), [this, &heightUniformDist]() { return heightUniformDist(engine); });
        }

        // Get all widths
        if (isWidthNormalDist)
        {
            StdNormDistGenerator widthNormDist(minWidth, maxWidth, engine);
            generate(widths.begin(), widths.end(), [&widthNormDist]() { return widthNormDist(); });
        }
        else
        {
            uniform_int_distribution<int> widthUniformDist(minWidth, maxWidth);
            generate(widths.begin(), widths.end(), [this, &widthUniformDist]() { return widthUniformDist(engine); });
        }

        vector<real> maturities(totalValuations);
        vector<real> meanReversionRates(totalValuations);
        transform(heights.begin(), heights.end(), maturities.begin(), [this](int height) { return (real)height / (termUnitsInYearCount * timeStep); });
        transform(widths.begin(), widths.end(), meanReversionRates.begin(), [this](int width) { return getWidthsFromTable(width); });

        while (currentNumOptions < totalValuations)
        {
            RandValuation val(maturities[currentNumOptions], timeStep, meanReversionRates[currentNumOptions], randYieldCurves.size(), false, noCashflows, onlyEuropean);
            addValuation(val, currentNumOptions);
        }
    ...
    void distribute_2()
    {
        ...
        while (currentNumOptions < totalValuations)
        {
            ...
            RandValuation val(50, timeStep, reversionRate, randYieldCurves.size(), false, noCashflows, onlyEuropean);
            addValuation(val, currentNumOptions);
        }
    ...
    void distribute_3()
    {
        ...
        while (currentNumOptions < totalValuations)
        {
            ...
            RandValuation val(maturity, timeStep, 0.01754, randYieldCurves.size(), false, noCashflows, onlyEuropean);
            addValuation(val, currentNumOptions);
        }
    ...
    void distribute_4_SKEWED(const int minHeight, const int maxHeight, const bool isHeightNormalDist, const int minWidth, const int maxWidth, const bool isWidthNormalDist, const int percentile, const bool inverse)
    {
        auto skewedPartCount = (int)(skewPercent / hundred * totalValuations);

        auto minSkewedHeight = minHeight + (int)ceil(percentile / hundred * (maxHeight - minHeight));
        auto maxSkewedHeight = maxHeight;
        auto minNormalHeight = minHeight;
        auto maxNormalHeight = minHeight + (int)ceil((hundred - percentile) / hundred * (maxHeight - minHeight));

        auto minSkewedWidth = minWidth + (int)ceil(percentile / hundred * (maxWidth - minWidth));
        auto maxSkewedWidth = maxWidth;
        auto minNormalWidth = minWidth;
        auto maxNormalWidth = minWidth + (int)ceil((hundred - percentile) / hundred * (maxWidth - minWidth));

        if (maxNormalWidth % 2 == 0) maxNormalWidth++; // Width needs to be odd
        ...
        if (!inverse)
        {
            if (isHeightNormalDist)
            {
                StdNormDistGenerator heightNormDistSkew(minSkewedHeight, maxSkewedHeight, engine);
                generate_n(heights.begin(), skewedPartCount, [&heightNormDistSkew]() { return heightNormDistSkew(); }); // Skewed Part (First)
                StdNormDistGenerator heightNormDist(minNormalHeight, maxNormalHeight, engine);
                generate(heights.begin() + skewedPartCount, heights.end(), [&heightNormDist]() { return heightNormDist(); }); // Normal Part (Remaining)
            }
            else
            {
                uniform_int_distribution<int> heightUniformDistSkew(minSkewedHeight, maxSkewedHeight);
                generate_n(heights.begin(), skewedPartCount, [this, &heightUniformDistSkew]() { return heightUniformDistSkew(engine); }); // Skewed Part (First)
                uniform_int_distribution<int> heightUniformDist(minNormalHeight, maxNormalHeight);
                generate(heights.begin() + skewedPartCount, heights.end(), [this, &heightUniformDist]() { return heightUniformDist(engine); }); // Normal Part (Remaining)
            }

            if (isWidthNormalDist)
            {
                StdNormDistGenerator widthNormDistSkew(minSkewedWidth, maxSkewedWidth, engine);
                generate_n(widths.begin(), skewedPartCount, [&widthNormDistSkew]() { return widthNormDistSkew(); }); // Skewed Part (First)
                StdNormDistGenerator widthNormDist(minNormalWidth, maxNormalWidth, engine);
                generate(widths.begin() + skewedPartCount, widths.end(), [&widthNormDist]() { return widthNormDist(); }); // Normal Part (Remaining)
            }
            else
            {
                uniform_int_distribution<int> widthUniformDistSkew(minSkewedWidth, maxSkewedWidth);
                generate_n(widths.begin(), skewedPartCount, [this, &widthUniformDistSkew]() { return widthUniformDistSkew(engine); }); // Skewed Part (First)
                uniform_int_distribution<int> widthUniformDist(minNormalWidth, maxNormalWidth);
                generate(widths.begin() + skewedPartCount, widths.end(), [this, &widthUniformDist]() { return widthUniformDist(engine); }); // Normal Part (Remaining)
            }
        }
        else
        {
            if (isHeightNormalDist)
            {
                StdNormDistGenerator heightNormDistSkew1(minNormalHeight, maxNormalHeight, engine);
                generate_n(heights.begin(), skewedPartCount, [&heightNormDistSkew1]() { return heightNormDistSkew1(); }); // Small Heights, Large Widths
                StdNormDistGenerator heightNormDistSkew2(minSkewedHeight, maxSkewedHeight, engine);
                generate_n(heights.begin() + skewedPartCount, skewedPartCount, [&heightNormDistSkew2]() { return heightNormDistSkew2(); }); // Large Heights, Small Widths
                StdNormDistGenerator heightNormDistNorm(minNormalHeight, maxNormalHeight, engine);
                generate(heights.begin() + 2 * skewedPartCount, heights.end(), [&heightNormDistNorm]() { return heightNormDistNorm(); }); // Normal Part (Remaining)
            }
            else
            {
                uniform_int_distribution<int> heightUniformDistSkew1(minNormalHeight, maxNormalHeight);
                generate_n(heights.begin(), skewedPartCount, [this, &heightUniformDistSkew1]() { return heightUniformDistSkew1(engine); }); // Small Heights, Large Widths
                uniform_int_distribution<int> heightUniformDistSkew2(minSkewedHeight, maxSkewedHeight);
                generate_n(heights.begin() + skewedPartCount, skewedPartCount, [this, &heightUniformDistSkew2]() { return heightUniformDistSkew2(engine); }); // Large Heights, Small Widths
                uniform_int_distribution<int> heightUniformDistNorm(minNormalHeight, maxNormalHeight);
                generate(heights.begin() + 2 * skewedPartCount, heights.end(), [this, &heightUniformDistNorm]() { return heightUniformDistNorm(engine); }); // Normal Part (Remaining)
            }

            if (isWidthNormalDist)
            {
                StdNormDistGenerator widthNormDistSkew1(minSkewedWidth, maxSkewedWidth, engine);
                generate_n(widths.begin(), skewedPartCount, [&widthNormDistSkew1]() { return widthNormDistSkew1(); }); // Small Heights, Large Widths
                StdNormDistGenerator widthNormDistSkew2(minNormalWidth, maxNormalWidth, engine);
                generate_n(widths.begin() + skewedPartCount, skewedPartCount, [&widthNormDistSkew2]() { return widthNormDistSkew2(); }); // Large Heights, Small Widths
                StdNormDistGenerator widthNormDistNorm(minNormalWidth, maxNormalWidth, engine);
                generate(widths.begin() + 2 * skewedPartCount, widths.end(), [&widthNormDistNorm]() { return widthNormDistNorm(); }); // Normal Part (Remaining)
            }
            else
            {
                uniform_int_distribution<int> widthUniformDistSkew1(minSkewedWidth, maxSkewedWidth);
                generate_n(widths.begin(), skewedPartCount, [this, &widthUniformDistSkew1]() { return widthUniformDistSkew1(engine); }); // Small Heights, Large Widths
                uniform_int_distribution<int> widthUniformDistSkew2(minNormalWidth, maxNormalWidth);
                generate_n(widths.begin() + skewedPartCount, skewedPartCount, [this, &widthUniformDistSkew2]() { return widthUniformDistSkew2(engine); }); // Large Heights, Small Widths
                uniform_int_distribution<int> widthUniformDistNorm(minNormalWidth, maxNormalWidth);
                generate(widths.begin() + 2 * skewedPartCount, widths.end(), [this, &widthUniformDistNorm]() { return widthUniformDistNorm(engine); }); // Normal Part (Remaining)
            }
        }
        ...
        vector<real> meanReversionRates(totalValuations);
        transform(heights.begin(), heights.end(), maturities.begin(), [this](int height) { return (real)height / (termUnitsInYearCount * timeStep); });
        //transform(widths.begin(), widths.end(), meanReversionRates.begin(), calculateMeanReversionRateFunctor(termUnitsInYearCount, timeStep));
        transform(widths.begin(), widths.end(), meanReversionRates.begin(), [this](int width) { return getWidthsFromTable(width); });
    ...
    void distribute_5()
    {
        ...
        while (currentNumOptions < (skewPercent / (real)100) * totalValuations)
        {
            ...
            RandValuation val(100, timeStep, reversionRate_skew, randYieldCurves.size(), true, noCashflows, onlyEuropean);
            addValuation(val, currentNumOptions);
        }
        ...
        while (currentNumOptions < totalValuations)
        {
            ...
            RandValuation val(maturity, timeStep, reversionRate, randYieldCurves.size(), false, noCashflows, onlyEuropean);
            addValuation(val, currentNumOptions);
        }
    ...
    void distribute_6()
    {
        ...
        while (currentNumOptions < (skewPercent / (real)100) * totalValuations)
        {
            ...
            RandValuation val(maturity, timeStep, reversionRate_skew, randYieldCurves.size(), true, noCashflows, onlyEuropean);
            addValuation(val, currentNumOptions);
        }
...
private:
    ...
    real getWidthsFromTable(int randWidth)
    {
        if (randWidth % 2 == 0)
            randWidth++;
        auto it = find(widths.begin(), widths.end(), randWidth);
    ...
    void addValuation(const RandValuation v, long &currentNumOptions)
    {
...
int main(int argc, char *argv[])
{
    ...
    int minWidth;
    int maxWidth;
    bool isHeightNormalDist;
    bool isWidthNormalDist;
    bool inverseSkewed;
    int percentile;
    ...
    minWidth = 7;
    maxWidth = 511;
    isHeightNormalDist = false;
    isWidthNormalDist = false;
    inverseSkewed = false;
    percentile = 90;
    ...
    cmd >> GetOpt::OptionPresent('c', "noCashflows", noCashflows);
    cmd >> GetOpt::OptionPresent('e', "onlyEuropean", onlyEuropean);
    cmd >> GetOpt::Option('i', "timeStep", timeStep);
    cmd >> GetOpt::Option('u', "termUnit", termUnit);
    ...
    cmd >> GetOpt::Option(NULL, "minHeight", minHeight);
    cmd >> GetOpt::Option(NULL, "maxHeight", maxHeight);
    cmd >> GetOpt::Option(NULL, "minWidth", minWidth);
    cmd >> GetOpt::Option(NULL, "maxWidth", maxWidth);
    cmd >> GetOpt::OptionPresent(NULL, "isHeightNormalDist", isHeightNormalDist);
    cmd >> GetOpt::OptionPresent(NULL, "isWidthNormalDist", isWidthNormalDist);
    cmd >> GetOpt::OptionPresent(NULL, "inverseSkewed", inverseSkewed);
    cmd >> GetOpt::Option(NULL, "percentile", percentile);
    ...
    options
        ...
        ("i,timeStep", "Time Step", cxxopts::value<int>(timeStep)->default_value("12"))
        ("u,termUnit", "Term Unit", cxxopts::value<int>(termUnit)->default_value("360"))
        ...
        ("minHeight", "Minimal Height", cxxopts::value<int>(minHeight)->default_value("12"))
        ("maxHeight", "Maximal Height", cxxopts::value<int>(maxHeight)->default_value("1200"))
        ("minWidth", "Minimal Width", cxxopts::value<int>(minWidth)->default_value("7"))
        ("maxWidth", "Maximal Width", cxxopts::value<int>(maxWidth)->default_value("511"))
        ("isHeightNormalDist", "Heights are drawn from Normal Distribution", cxxopts::value<bool>(isHeightNormalDist)->default_value("false"))
        ("isWidthNormalDist", "Widths are drawn from Normal Distribution", cxxopts::value<bool>(isWidthNormalDist)->default_value("false"))
        ("inverseSkewed", "'skewPercent' of Small Heights skewed together with Large Widths, and 'skewPercent' the opposite, the remaining normal.", cxxopts::value<bool>(inverseSkewed)->default_value("false"))
        ("percentile", "Percentile of Heights/Widths that should be the lower/upper boundary of skewed/normal, e.g. top 90 for max of 1200 is 1060 and bottom 10 is 120", cxxopts::value<int>(percentile)->default_value("90"))
        ;
    ...
    RandomDataGenerator rdg(randomSeed, totalValuations, totalYieldCurves, outputPath, timeStep, termUnit, skewPercent, noCashflows, onlyEuropean);

    if (minHeight > maxHeight)
    {
        stringstream ss;
        ss << "Wrong Heights: minHeight > maxHeight: " << minHeight << " > " << maxHeight;
    ...
    {
        stringstream ss;
        ss << "Wrong Widths: minWidth > maxWidth: " << minWidth << " > " << maxWidth;
        throw std::invalid_argument(ss.str());
    }
    ...
    {
    case 0:
        rdg.distribute_0_UNIFORM(600, 253);
        break;
    case 1:
        rdg.distribute_1_RAND(minHeight, maxHeight, isHeightNormalDist, minWidth, maxWidth, isWidthNormalDist);
        break;
    case 2:
        rdg.distribute_2();
        break;
    case 3:
        ...
        break;
    case 4:
        rdg.distribute_4_SKEWED(minHeight, maxHeight, isHeightNormalDist, minWidth, maxWidth, isWidthNormalDist, 90, inverseSkewed);
        break;
    case 5:
        rdg.distribute_5();
        break;
    ...
    default:
        ...
        rdg.distribute_0_UNIFORM(minHeight, minWidth);
        rdg.distribute_1_RAND(minHeight, maxHeight, isHeightNormalDist, minWidth, maxWidth, isWidthNormalDist);
        rdg.distribute_2();
        rdg.distribute_3();
        rdg.distribute_4_SKEWED(minHeight, maxHeight, isHeightNormalDist, minWidth, maxWidth, isWidthNormalDist, 90, inverseSkewed);
